...
internal class Blur : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture sourcert, RenderTexture destinationrt)
    {
        ...
        blurMaterial.SetVector(
            "_Param",
            new Vector4(radius * widthModification, -radius * widthModification, 0f, 0f)
        );
        ...
        for (int i = 0; i < qualityIterations; i++)
        {
            ...
            blurMaterial.SetVector(
                "_Param",
                new Vector4(
                    (radius * widthModification) + iterationOffset,
                    (-radius * widthModification) - iterationOffset,
                    0.0f,
                    0.0f
                )
            );
            RenderTexture rendertexturetemp = RenderTexture.GetTemporary(
                rendertextureWidth,
                rendertextureHeight,
                0,
                sourcert.format
